            ...
            <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-6">
                    {features.map((feature, index) => (
                        <div key={index} className="flex flex-col items-center text-center p-4 md:p-6 rounded-2xl bg-slate-50 border border-slate-100 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                            <div className={`w-14 h-14 md:w-20 md:h-20 rounded-2xl ${feature.bgColor} ${feature.borderColor} border flex items-center justify-center mb-4 md:mb-6 shadow-sm`}>
                                {feature.icon}
                            </div>
